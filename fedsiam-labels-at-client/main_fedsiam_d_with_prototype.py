...
from models.Update import LocalUpdate, PrototypeModule
...
if __name__ == '__main__':
    ...
    if args.dataset == 'mnist':
        trans_mnist_ema = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((0.1307,), (0.3081,))
        ])
        dataset_train = datasets.MNIST(
            '../data/mnist/', train=True, download=True, transform=trans_mnist_ema
        )
        dataset_train_ema = datasets.MNIST(
            '../data/mnist/', train=True, download=True, transform=trans_mnist_ema
        )
        dataset_test = datasets.MNIST(
            '../data/mnist/', train=False, download=True, transform=trans_mnist_ema
        )
        ...
        dataset_train_ema = datasets.CIFAR10(
            '../data/cifar', train=True, download=True, transform=trans_cifar
        )
        ...
        dataset_train = datasets.SVHN(
            '../data/svhn', split='train', download=True, transform=trans_svhn_test
        )
        dataset_train_ema = datasets.SVHN(
            '../data/svhn', split='train', download=True, transform=trans_svhn_test
        )
        dataset_test = datasets.SVHN(
            '../data/svhn', split='test', download=True, transform=trans_svhn_test
        )
    else:
    ...
    if args.iid == 'noniid_ssl' and args.dataset == 'cifar':
        ...
        dict_users, dict_users_labeled, pseudo_label = sample(
            dataset_train, args.num_users, args.label_rate, args.iid
        )
    ...
    for iter in range(args.epochs):
        ...
        # Garante que o mentor esteja na rodada
        if mentor_client_id not in idxs_users:
            idxs_users[-1] = mentor_client_id
        
        # Processa o mentor primeiro, para atualizar protótipos
        idxs_users_sorted = sorted(
            idxs_users, key=lambda x: 0 if x == mentor_client_id else 1
        )
        ...
        for idx in idxs_users_sorted:
            ...
            # Injetar módulo de PROTOTYPES no cliente
            local.prototype_module = proto_module
            local.client_id = idx
        ...
        print(
            'Round {:3d}, loss {:.3f}, acc_valid {:.2f}%, acc_ema_valid {:.2f}%, acc_train {:.2f}%'
